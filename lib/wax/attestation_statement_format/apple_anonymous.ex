defmodule Wax.AttestationStatementFormat.AppleAnonymous do
  ...
  def verify(
    ...
    with :ok <- valid_cbor?(att_stmt),
         ...
         :ok <- check_certificate_path(att_stmt)
    do
      {:ok, {:anonymous, att_stmt["x5c"], nil}}
    end
  end

  defp valid_cbor?(%{"x5c" => _}), do: :ok
  defp valid_cbor?(_), do: {:error, :attestation_tpm_invalid_cbor}

  defp check_vertificate_validity(cert) do
    if Wax.Utils.Certificate.version(cert) == :v3
      and Wax.Utils.Certificate.basic_constraints_ext_ca_component(cert) == false do
      :ok
    else
      {:error, :attestation_apple_invalid_cert}
    end
  end
  ...
  defp check_nonces_match(cert, auth_data, client_data_hash) do
    ...
    {:Extension, _oid, _, <<_::binary-size(6), nonce::binary>>} =
      X509.Certificate.extension(cert, {1, 2, 840, 113635, 100, 8, 2})

    if nonce == expected_nonce, do: :ok, else: {:error, :attestation_apple_invalid_nonce}
  end
  ...
  defp check_keys_match(auth_data, att_cert) do
    ...
    if att_cert_erlang_public_key == auth_data_erlang_public_key do
      ...
      {:error, :attestation_apple_public_key_mismatch}
    end
  end
  ...
  defp check_certificate_path(att_stmt) do
    ...
    case :public_key.pkix_path_validation(@apple_root_cert_der, certs_der, []) do
      ...
      {:error, _} ->
        {:error, :attestation_apple_path_validation_failed}
    end
  end
end
